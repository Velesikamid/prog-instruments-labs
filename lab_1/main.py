...
def main():
    ...
    for key in manual_results.keys():
        manual = manual_results[key]
        builtin = builtin_results[key]
        print(f"{key:15}\t{manual:.6f}\t\t{builtin:.6f}")
    ...
    print("- 2-4 интервала: слишком мало,",
          "теряем информацию о форме распределения")
    print("- 20-25 интервалов: слишком много, появляется излишняя детализация")
    print(f"Оптимальное число: {len(abs_freq)}",
          "интервалов (по правилу Стёрджеса)")
    ...
    rel_freq_Y, bin_edges_Y = np.histogram(
        Y,
        bins=len(abs_freq),
        density=False
    )
